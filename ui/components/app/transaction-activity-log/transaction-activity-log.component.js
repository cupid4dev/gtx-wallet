...
export default class TransactionActivityLog extends PureComponent {
  ...
  renderActivity (activity, index) {
    ...
    const { eventKey, value, value2, timestamp, hash } = activity

    let ethValue = index === 0
      ? `${getValueFromWeiHex({
        value,
        fromCurrency: 'ETH',
        toCurrency: 'ETH',
    ...
    ethValue = ethValue.replace('ETH', nativeCurrency)
    let values = ethValue
    if (index === 0 && value2 && value2 !== '0x0' && value2 !== '0x00') {
      const thetaValue = `${getValueFromWeiHex({
        value: value2,
        fromCurrency: 'ETH',
        toCurrency: 'ETH',
        numberOfDecimals: 6,
      })} ${THETA_SYMBOL}`
      values = `${ethValue} + ${thetaValue}`
    }
    const formattedTimestamp = formatDate(timestamp, 'T \'on\' M/d/y')
    const activityText = this.context.t(eventKey, [values, formattedTimestamp])
