...
import { GANACHE_CHAIN_ID, GANACHE_NETWORK_ID, KOVAN_CHAIN_ID, KOVAN_NETWORK_ID, MAINNET_CHAIN_ID, MAINNET_NETWORK_ID, RINKEBY_CHAIN_ID, RINKEBY_NETWORK_ID, ROPSTEN_CHAIN_ID, ROPSTEN_NETWORK_ID, THETAMAINNET_CHAIN_ID, THETAMAINNET_NETWORK_ID } from '../../../app/scripts/controllers/network/enums'
...
export function isEthNetwork (netIdOrChainId) {
  return [
    ...
    GANACHE_NETWORK_ID, GANACHE_CHAIN_ID,
  ].includes(netIdOrChainId)
}

export function isThetaNetwork (netIdOrChainId) {
  return [
    THETAMAINNET_NETWORK_ID, THETAMAINNET_CHAIN_ID,
  ].includes(netIdOrChainId)
}

export function isTokenIrremovable (tokenAddress, isNativeAsset = false) {
  const lcTokenAddress = tokenAddress?.toLowerCase()
  return isNativeAsset ||
    Object.values(gtxTokens).find((t) => t.address.toLowerCase() === lcTokenAddress)?.autoAdd ||
    Object.values(thetaTokens).find((t) => t.address.toLowerCase() === lcTokenAddress)?.autoAdd
}
