...
export default function ConfirmApprove () {
  ...
  const { tokensWithBalances = [] } = useTokenTracker(currentToken?.address ? [currentToken] : [])
  ...
  if (isInternal) {
    const tokenEx = Object.values(mergedTokens).find(({ address }) => address?.toLowerCase() === currentToken?.address)
    if (tokenEx?.logo) {
