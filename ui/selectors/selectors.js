...
import { NETWORK_TYPE_TO_ID_MAP, THETAMAINNET_CHAIN_ID, THETAMAINNET_NETWORK_ID } from '../../app/scripts/controllers/network/enums'
...
export function getAverageGasPriceParams (state, eip1559) {
  ...
  return {
    ...(useEip1559 && {
      maxFeePerGas: addHexPrefix(decGWEIToHexWEI(((gasPricing.baseFee || 0) * baseFeeMultiplier) + (gasPricing.average))),
      maxPriorityFeePerGas: addHexPrefix(decGWEIToHexWEI(gasPricing.average)),
    }),
    ...(!useEip1559 && {
      gasPrice: addHexPrefix(decGWEIToHexWEI(gasPricing.average + (gasPricing.baseFee ?? 0))),
    }),
  }
}
...
export function getIsThetaMainnet (state) {
  ...
  return (chainId === THETAMAINNET_CHAIN_ID || chainId === THETAMAINNET_NETWORK_ID)
}
...
export function getShouldShowFiat (state) {
  ...
  return Boolean((isMainNet || isThetaMainNet || showFiatInTestnets) && conversionRate)
}
