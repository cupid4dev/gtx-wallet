...
import { THETAMAINNET_CHAIN_ID, THETAMAINNET_DISPLAY_NAME, THETAMAINNET_NETWORK_ID, THETASC_DISPLAY_NAME, THETA_NATIVE_COLOR, THETA_SC_COLOR } from '../../../../app/scripts/controllers/network/enums'
...
function mapDispatchToProps (dispatch) {
  return {
    ...
    setRpcTarget: (target, networkId, ticker, nickname, rpcPrefs) => {
      dispatch(actions.setRpcTarget(target, networkId, ticker, nickname, rpcPrefs))
    },
    delRpcTarget: (target) => {
...
class NetworkDropdown extends Component {
  ...
  renderCustomOption (provider) {
    ...
    const thetaNet = defaultNetworksData.find((e) => e.labelKey === 'theta_mainnet')
    ...
    switch (rpcTarget) {
      ...
      default:
        return (
          <DropdownMenuItem
            key={rpcTarget}
            onClick={() => this.props.setRpcTarget(rpcTarget, network, ticker, nickname, rpcPrefs)}
  ...
  renderCommonRpc (rpcListDetail, provider) {
    ...
    return reversedRpcListDetail.map((entry) => {
      const { rpcUrl: rpc, chainId, ticker = 'ETH', nickname = '', rpcPrefs = {} } = entry
      const currentRpcTarget = provider.type === 'rpc' && rpc === provider.rpcTarget
      if (chainId === THETAMAINNET_CHAIN_ID || chainId === THETAMAINNET_NETWORK_ID || rpc === 'http://localhost:8545' || currentRpcTarget) {
        return null
      }
      return (
        <DropdownMenuItem
          key={`common${rpc}`}
          closeMenu={() => this.props.hideNetworkDropdown()}
          onClick={() => this.props.setRpcTarget(rpc, chainId, ticker, nickname, rpcPrefs)}
          style={{
            fontSize: '16px',
            lineHeight: '20px',
            padding: '12px 0',
  ...
  render () {
    ...
    const thetaNet = defaultNetworksData.find((e) => e.labelKey === 'theta_mainnet')
    const onTheta = providerType === thetaNet.providerType && activeNetwork === thetaNet.rpcUrl
      ...
      >
        ...
        <DropdownMenuItem
          ...
          onClick={() => {
            ...
            this.props.setRpcTarget(thetaNet.rpcUrl, thetaNet.chainId, thetaNet.ticker, THETAMAINNET_DISPLAY_NAME, {
        ...
        >
          ...
          <NetworkDropdownIcon backgroundColor={THETA_NATIVE_COLOR} isSelected={onTheta} />
          <span
            className="network-name-item"
            style={{
              color: onTheta
        ...
        <DropdownMenuItem
          ...
          onClick={() => {
            ...
            this.props.setRpcTarget(thetaNet.rpcUrl, thetaNet.chainId, thetaNet.ticker, THETASC_DISPLAY_NAME, {
              blockExplorerUrl: thetaNet.blockExplorerUrl,
        ...
        >
          ...
          <NetworkDropdownIcon backgroundColor={THETA_SC_COLOR} isSelected={onTheta} />
          <span
            className="network-name-item"
            style={{
              color: onTheta
        ...
        >
          ...
          <NetworkDropdownIcon backgroundColor="#454a75" isSelected={providerType === 'mainnet'} />
          <span
            className="network-name-item"
            style={{
