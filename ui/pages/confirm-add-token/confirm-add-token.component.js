...
export default class ConfirmAddToken extends Component {
  ...
  render () {
    const { history, addTokens, clearPendingTokens, mostRecentOverviewPage, pendingTokens, assetImages } = this.props
            ...
            <div className="confirm-add-token__token-list">
              {
                Object.entries(pendingTokens)
                  .map(([address, token]) => {
                    ...
                    const lcAddress = address.toLocaleLowerCase()
                    const image = token.logo ?? assetImages?.[lcAddress] ?? lcDefaultTokens[lcAddress]?.image ?? lcDefaultTokens[lcAddress]?.logo
                    token.logo = image
