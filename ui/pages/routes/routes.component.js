...
import WrappableAsset from '../wrappable-asset'
import StakeAsset from '../stake-asset'
...
import { NETWORK_TYPE_TO_ID_MAP, THETAMAINNET_NETWORK_ID, networkColor } from '../../../app/scripts/controllers/network/enums'
...
export default class Routes extends Component {
  ...
  renderRoutes () {
    ...
    const routes = (
      <Switch>
        ...
        <Authenticated path={`${WRAP_ASSET_ROUTE}/:asset`} component={WrappableAsset} />
        <Authenticated path={`${WRAP_ROUTE}`} component={Wrap} />
        ...
        <Authenticated path={STAKE_ROUTE} component={Stake} />
        <Authenticated path={`${STAKE_ASSET_ROUTE}/:asset`} component={StakeAsset} />
  ...
  render () {
    ...
    const chainId = NETWORK_TYPE_TO_ID_MAP[provider.type]?.chainId || provider.chainId
    const netColor = networkColor(chainId, selectedNative)
      ...
      >
        ...
        { !this.hideAppHeader() && (
          <AppHeader
            ...
            disabled={this.onConfirmPage() || isEditing}
          />
        ) }
        <Sidebar
        ...
        />
        <AccountMenu />
        <div className="main-container-wrapper" style={{ 'backgroundColor': netColor }} >
  ...
  getConnectingLabel (loadingMessage) {
    ...
    switch (provider.type) {
      ...
      default:
        if (provider?.chainId === THETAMAINNET_NETWORK_ID) {
          return this.context.t('connectingToThetaMainnet')
        }
  ...
  getNetworkName () {
    switch (this.props.provider.type) {
      ...
      // case 'theta_mainnet':
      //  return this.context.t('theta_mainnet')
      case 'ropsten':
      ...
      default:
        if (this.props.provider?.chainId === THETAMAINNET_NETWORK_ID) {
          return this.context.t('theta_mainnet')
        }
