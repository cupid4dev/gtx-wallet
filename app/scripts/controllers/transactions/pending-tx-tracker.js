...
import { THETAMAINNET_NATIVE_RPC_URL, THETAMAINNET_NETWORK_ID, THETA_GASPRICE_HEXWEI } from '../network/enums'
...
export default class PendingTransactionTracker extends EventEmitter {
  ...
  */
  ...
  async _checkPendingTx (txMeta) {
    ...
    try {
      if (txMeta.txParams.isThetaNative) { // TODO: may want to move this requester and parser into transaction utils like isSmartContract
        ...
        const txInfo = await new Promise((resolve, reject) => {
          request({
            ...
            if (err) {
              log.error(`ERROR fetching theta tx with hash ${txHash}: `, JSON.stringify(err))
              reject(err)
              return
            }
            log.debug(res)
        ...
        if (txInfo.status === 'finalized' && txInfo.type && parseInt(txInfo.block_height, 10)) {
          const transactionReceipt = {
            blockHash: txInfo.block_hash,
            blockNumber: txInfo.block_height,
            contractAddress: txInfo.receipt?.ContractAddress || undefined,
            // cumulativeGasUsed,
            from: txInfo.transaction?.from?.address || txInfo.transaction?.inputs?.[0]?.address,
            ...
            gasUsed: txInfo.blance_changes?.GasUsed
              ...
              : `0x${(new BN(txInfo.transaction?.fee?.tfuelwei, 10).div(baseFeePerGas)).toString('hex')}`,
            logs: txInfo.receipt?.Logs,
            // logsBloom,
            // root,
            to: txInfo.transaction?.to?.address || txInfo.transaction?.outputs?.[0]?.address,
            transactionHash: txInfo.hash || txInfo.receipt?.TxHash,
            transactionIndex: (parseInt(txInfo.transaction?.from?.sequence || txInfo.transaction?.inputs?.[0].sequence, 10) - 1) || undefined,
            isThetaNative: true,
          }
          this.emit('tx:confirmed', txId, transactionReceipt, `0x${baseFeePerGas.toString('hex')}`)
          return
        }
      } else { // not theta-native:
