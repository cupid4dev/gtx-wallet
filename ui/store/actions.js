...
export function setSelectedNFT (value) {
  return async (dispatch) => {
    await promisifiedBackground.setSelectedNFT(value)
    await forceUpdateMetamaskState(dispatch)
  }
}

export function setSelectedNative (value) {
  return async (dispatch) => {
    await promisifiedBackground.setSelectedNative(value)
    await forceUpdateMetamaskState(dispatch)
  }
}
...
// prepares the tx data creating an unapproved transaction triggering processing in the metamask controller which then triggers the UI update of the confirm page
...
export function signTokenTx (tokenAddress, toAddress, amount, txData) {
  return (dispatch) => {
    dispatch(showLoadingIndication())
    const token = global.eth.contract(erc20Abi).at(tokenAddress)
...
export function signNftTx (tokenAddress, fromAddress, toAddress, tokenID, txData) {
  return (dispatch) => {
    dispatch(showLoadingIndication())
    const token = global.eth.contract(erc721Abi).at(tokenAddress)
    token.transferFrom(fromAddress, toAddress, tokenID, txData)
      .catch((err) => {
        dispatch(hideLoadingIndication())
        dispatch(displayWarning(err.message))
      })
...
export function setGasPriceParams (gasPriceParams) {
  return {
    type: actionConstants.UPDATE_GAS_PRICE_PARAMS,
    value: gasPriceParams,
  }
}
...
export function updateGasData ({
  ...
  return (dispatch) => {
    ...
    return estimateGas({
      ...
      to,
      value,
      estimateGasPrice: (gasPriceParams?.maxFeePerGas ?? gasPriceParams?.gasPrice) || '0x0',
      data,
      network,
      selectedNative,
    })
...
export function updateSendTokenBalance ({
  ...
  return (dispatch) => {
    ...
    return tokenBalancePromise
      .then((usersToken) => {
        if (usersToken) {
          const newTokenBalance = calcTokenBalance({ sendAsset, usersToken })
...
export function updateSendAsset (asset) {
  return {
    type: actionConstants.UPDATE_SEND_ASSET,
    value: asset,
  }
}
...
export function completedTx (id) {
  return (dispatch, getState) => {
    ...
    const {
      ...
      network,
      provider: { chainId, rpcPrefs: { selectedNative } = {} } = {},
    } = state.metamask
    const unconfirmedActions = txHelper(unapprovedTxs, unapprovedMsgs, unapprovedPersonalMsgs, null, null, unapprovedTypedMessages, network, chainId, selectedNative)
...
export function addToken (address, symbol, decimals, image) {
  return (dispatch, getState) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      background.addToken(address, symbol, decimals, image, chainId, (err, tokens) => {
        dispatch(hideLoadingIndication())
        if (err) {
          dispatch(displayWarning(err.message))
          reject(err)
...
export function addTokens (tokens) {
  return (dispatch) => {
    if (Array.isArray(tokens)) {
      return Promise.all(tokens.map(({ address, symbol, decimals, image, logo }) => (
        dispatch(addToken(address, symbol, decimals, image ?? logo))
      )))
    }
    return Promise.all(
      Object
        .entries(tokens)
        .map(([_, { address, symbol, decimals, image, logo }]) => (
          dispatch(addToken(address, symbol, decimals, image ?? logo))
        )),
    )
...
export function createCancelTransaction (txId, customGasPriceParams) {
  log.debug('background.cancelTransaction')
  let newTxId

  return (dispatch) => {
    return new Promise((resolve, reject) => {
      background.createCancelTransaction(txId, customGasPriceParams, (err, newState) => {
        if (err) {
          dispatch(displayWarning(err.message))
          reject(err)
          return
...
export function createSpeedUpTransaction (txId, customGasPriceParams, customGasLimit) {
  log.debug('background.createSpeedUpTransaction')
  let newTx

  return (dispatch) => {
    return new Promise((resolve, reject) => {
      background.createSpeedUpTransaction(txId, customGasPriceParams, customGasLimit, (err, newState) => {
        if (err) {
          dispatch(displayWarning(err.message))
          reject(err)
          return
...
export function createRetryTransaction (txId, customGasPriceParams, customGasLimit) {
...
export function changeSelectedNFT () {
  return {
    type: actionConstants.SELECTED_NFT_CHANGED,
  }
}
...
export function setPendingTokens (pendingTokens) {
  ...
  const tokens = address && symbol && decimals >= 0 && decimals <= 36
...
/**
 * Asks user where to save backup file and saves it.
 */
export function backupData () {
  return () => {
    background.backupData()
  }
}

/**
 * Restores the user data state to the given state, excluding identities.
 */
export function restoreData (json) {
  return () => {
    background.restoreData(json)
  }
}
