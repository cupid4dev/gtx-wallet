...
export function getActivities (transaction, isFirstTransaction = false) {
  const {
    ...
    hash,
    history = [],
    txParams: { gas: paramsGasLimit, gasPrice: paramsGasPrice, maxFeePerGas: paramsMaxFeePerGas },
    xReceipt: { status } = {},
    type,
  } = transaction
  ...
  const historyActivities = history.reduce((acc, base, index) => {
    ...
    if (index === 0 && !Array.isArray(base) && base.txParams) {
      const { time: timestamp, txParams: { value, value2, gas = '0x0', gasPrice, maxFeePerGas } = {} } = base
      ...
      cachedGasLimit = gas
      cachedGasPrice = maxFeePerGas ?? gasPrice
            ...
            case STATUS_PATH: {
              const gasFee = (cachedGasLimit === '0x0' && cachedGasPrice === '0x0')
                ? getHexGasTotal({
                  ...
                  gasPrice: paramsMaxFeePerGas ?? paramsGasPrice,
                })
                : getHexGasTotal({
              ...
              if (value in statusHash) {
                ...
                if (value === SUBMITTED_STATUS) {
                  ...
                  } else if (type === TRANSACTION_TYPE.CANCEL) {
