...
export const incomingTxListSelector = (state) => {
  ...
  return Object.values(state.metamask.incomingTransactions)
    .filter((tx) => (
      ...
      transactionMatchesNetwork(tx, chainId, network, selectedNative)
    ))
}
...
export const nonceSortedTransactionsSelector = createSelector(
  ...
  (transactions = []) => {
    ...
    transactions.forEach((transaction) => {
      const { txParams: { nonce } = {}, status, time: txTime, type } = transaction

      if (typeof nonce === 'undefined' || type === TRANSACTION_TYPE.INCOMING) {
        ...
        if (type === TRANSACTION_TYPE.RETRY) {
        ...
        nonceToTransactionsMap[nonce] = {
          ...
          initialTransaction: transaction,
          primaryTransaction: transaction,
          hasRetried: transaction.type === TRANSACTION_TYPE.RETRY,
          hasCancelled: transaction.type === TRANSACTION_TYPE.CANCEL,
        }
