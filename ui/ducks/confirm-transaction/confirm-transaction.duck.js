...
export default function reducer (state = initState, action = {}) {
  switch (action.type) {
    ...
    case UPDATE_TOKEN_PROPS: {
      const { symbol = '', decimals = '' } = action.payload
      return {
        ...state,
        tokenProps: {
          ...state.tokenProps,
...
export function updateGasAndCalculate ({ gasLimit, gasPriceParams }) {
  return (dispatch, getState) => {
    const { confirmTransaction: { txData } } = getState()
    const useGasPriceParams = {
      ...(gasPriceParams.maxFeePerGas && {
        maxFeePerGas: addHexPrefix(gasPriceParams.maxFeePerGas),
        maxPriorityFeePerGas: addHexPrefix(gasPriceParams.maxPriorityFeePerGas),
      }),
      ...(!gasPriceParams.maxFeePerGas && {
        gasPrice: addHexPrefix(gasPriceParams.gasPrice),
      }),
    }
    const newTxData = {
      ...txData,
...
function increaseFromLastGasPrice (txData) {
  const { lastGasPriceParams, txParams: { gasPrice: previousGasPrice, maxFeePerGas: previousMaxFeePerGas, maxPriorityFeePerGas: previousMaxPriorityFeePerGas } = {} } = txData

  // Set the minimum to a 10% increase from the last
  const gasPriceParams = {}
  if (previousMaxFeePerGas) {
    const minimumFeePerGas = increaseLastGasPrice(lastGasPriceParams?.maxFeePerGas)
    const feeBelowMinimum = hexGreaterThan(minimumFeePerGas, previousMaxFeePerGas)
    gasPriceParams.maxFeePerGas = (!previousMaxFeePerGas || feeBelowMinimum) ? minimumFeePerGas : previousMaxFeePerGas

    const minimumPriorityFee = increaseLastGasPrice(lastGasPriceParams?.maxPriorityFeePerGas)
    const priorityFeeBelowMinimum = hexGreaterThan(minimumPriorityFee, previousMaxPriorityFeePerGas)
    gasPriceParams.maxPriorityFeePerGas = (!previousMaxPriorityFeePerGas || priorityFeeBelowMinimum) ? minimumPriorityFee : previousMaxPriorityFeePerGas
  } else {
    const minimumGasPrice = increaseLastGasPrice(lastGasPriceParams?.gasPrice)
    const gasPriceBelowMinimum = hexGreaterThan(minimumGasPrice, previousGasPrice)
    gasPriceParams.gasPrice = (!previousGasPrice || gasPriceBelowMinimum) ? minimumGasPrice : previousGasPrice
  }
...
export function updateTxDataAndCalculate (txData) {
  return (dispatch, getState) => {
    ...
    const { txParams: { value = '0x0', gas: gasLimit = '0x0', gasPrice = '0x0', maxFeePerGas } = {} } = txData
...
export function setTransactionToConfirm (transactionId) {
  return (dispatch, getState) => {
    ...
    if (transaction.txParams) {
      const { lastGasPriceParams } = transaction
      const txData = lastGasPriceParams ? increaseFromLastGasPrice(transaction) : transaction
      ...
      if (txParams.data) {
        ...
        const { symbol, decimals } = tokens?.find(({ address }) => address === txParams.to) ?? {}
        dispatch(updateTokenProps({ symbol, decimals }))
      }
